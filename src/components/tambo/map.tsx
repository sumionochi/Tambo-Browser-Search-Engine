...
const ClusterGroup: React.FC<MarkerClusterGroupProps> = ({
  ...
  React.useEffect(() => {
    ...
    React.Children.forEach(children, (child) => {
      if (React.isValidElement<MarkerProps>(child) && child.props.position) {
        // Extract marker options, excluding children
        const { children: markerChildren, ...markerOptions } = child.props;
        ...
        const tooltipChild = React.Children.toArray(markerChildren).find(
          (tooltipChild) =>
            React.isValidElement(tooltipChild) && tooltipChild.type === Tooltip,
        );

        if (React.isValidElement<TooltipProps>(tooltipChild)) {
          const tooltipProps = tooltipChild.props;
          marker.bindTooltip(tooltipProps.children?.toString() || '', {
            direction: tooltipProps.direction ?? 'auto',
            permanent: tooltipProps.permanent ?? false,
            sticky: tooltipProps.sticky ?? false,
            opacity: tooltipProps.opacity ?? 0.9,
          });
        }
